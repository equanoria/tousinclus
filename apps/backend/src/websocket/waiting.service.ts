...
export class WaitingService {
    ...
    async handleWaitingLogic(server: Server, client: Socket, data: any): Promise<void> {
        ...
        switch (action) {
            case 'handle-team':
                ...
                await this.handleTeamConnection(server, client, data.gameCode, data.team, client.id);
                break;
    ...
    async handleTeamConnection(server, client: Socket, gameCode: string, team: string, clientId: string): Promise<void> {
        try {
            ...
            const isReadyToStart = await this.gameService.checkIfReadyToStart(gameCode)

            if (isReadyToStart) {
                // Envoie un message à tout les participants de la room
                server.to(gameCode).emit('game-status', { gameStatus: 'start', gameCode });
